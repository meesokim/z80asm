...
void pass2(void), p2_file(char *), p3_file(char *, int);
...
void p3_file(char *fn, int len)
{
	...
	strcpy(d, ".inc");
	//printf("include = %s, %d\n", incfile, len);
	fp = fopen(incfile, WRITEA);
	while (i < len) {
		fprintf(fp, "%-8s\tEQU 0%04XH\n", symarray[i]->sym_name, symarray[i]->sym_val & 0xffff);
		//printf("%-8s\tEQU 0%04XH\n", symarray[i]->sym_name, symarray[i]->sym_val & 0xffff);
		i++;
	}
	fclose(fp);
}
...
comment:
	*s = '\0';
	if (i > 0 && *l != LABSEP && *ss != '#' && strcmp (ss, "ORG") == 0)
	{
		opcheck = 1;
		printf("opcode - %s\n", ss);
	}
