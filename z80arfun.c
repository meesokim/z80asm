...
/*
 *	process IX/IY opcodes fpr 4 bytes
 */
int idxreg4(char *p, int v3)
{
	int len = 4;
	if (strncmp(p, "(IX+", 4) == 0 || strncmp(p, "(IX-", 4) == 0) {
		if (pass == 2) {
			ops[0] = 0xdd;
			ops[1] = 0xcb;
			ops[2] = (p[3] == '+' ? 1 : -1) * chk_v2(calc_val(strchr(p, p[3]) + 1));
			ops[3] = v3;
		}
	} else if (strncmp(p, "(IY+", 4) == 0 || strncmp(p, "(IY-", 4) == 0) {
		if (pass == 2) {
			ops[0] = 0xfd;
			ops[1] = 0xcb;
			ops[2] = (p[3] == '+' ? 1 : -1) * chk_v2(calc_val(strchr(p, p[3]) + 1));
...
/*
 *	process IX/IY opcodes fpr 3 bytes
 */
int idxreg3(char *p, int v1)
{
	int len = 3;
	if (strncmp(p, "(IX+", 4) == 0 || strncmp(p, "(IX-", 4) == 0 || strncmp(p, "(IX)", 4) == 0) {
		if (pass == 2) {
			ops[0] = 0xdd;
			ops[1] = v1;
			if (p[3] == ')')
	...
	} else if (strncmp(p, "(IY+", 4) == 0 || strncmp(p, "(IY-", 4) == 0 || strncmp(p, "(IY)", 4) == 0) {
		if (pass == 2) {
			ops[0] = 0xfd;
			ops[1] = v1;
			if (p[3] == ')')
...
int op_ld(void)
{
	...
	*p2 = '\0';
	...
	case NOOPERA:			/* missing operand */
		...
		break;
	default:
		if (strncmp(operand, "(IX+", 4) == 0 || strncmp(operand, "(IX-", 4) == 0 || strncmp(operand, "(IX)", 4) == 0)
			len = ldiix();	/* LD (IX+d),? */
		else if (strncmp(operand, "(IY+", 4) == 0 || strncmp(operand, "(IY-", 4) == 0 || strncmp(operand, "(IY)", 4) == 0)
			len = ldiiy();	/* LD (IY+d),? */
		else if (*operand == '(')
			len = ldinn();	/* LD (nn),? */
	...
	case REGL:			/* LD (IX+d),L */
		...
		if (pass == 2) {
			...
			if (v == ')')
				...
				ops[2] = (v == '+' ? 1 : -1) * chk_v2(calc_val(strchr(operand, v) + 1));
		}
		break;
...
int op_sub(void)
{
	...
	case REGIHL:			/* SUB (HL) */
		...
		break;
	case NOREG:			/* operand isn't register */
		len = idxreg3(operand, 0x96);
		if (len == 2 && pass == 2) {
			ops[0] = 0xd6;
			ops[1] = chk_v2((char)eval(operand));
		}		
		break;
	case NOOPERA:			/* missing operand */
		len = 1;
